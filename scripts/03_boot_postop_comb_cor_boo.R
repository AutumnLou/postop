...
haz <- data.frame(matrix(ncol = length(rownames(cor_df)), nrow = 0))
...
for(i in 1:length(bootIndex)) {
  ...
  # Boost data prep
  x <- model.matrix( ~ .-1, bag[,c(1:8,11:ncol(bag))])
  y <- Surv(time = bag$BCR_FreeTime,
            event = bag$BCR_Event)
  test_x <- model.matrix( ~ .-1, oob[,c(1:8,11:ncol(oob))])
...
write.csv(selection.v, 'data/var_select_comb_cor_boo.csv')
