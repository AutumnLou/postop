...
for(i in 1:length(bootIndex)) {
  ...
  # Boost data prep
  x <- model.matrix( ~ .-1, bag_gene_data[,c(1:8,11:ncol(bag_gene_data))])
  y <- Surv(time = bag_gene_data$BCR_FreeTime,
            event = bag_gene_data$BCR_Event)
  test_x <- model.matrix( ~ .-1, oob_gene_data[,c(1:8,11:ncol(oob_gene_data))])
...
write.csv(selection.v, 'data/var_select_comb_cor_uni_boo.csv')
