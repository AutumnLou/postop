...
# MSKCC
## Bootstrapped
# 04_boot_msk_postop_nomo.R
###########################################################################

source("scripts/03_msk_postop_nomo.R")
source("scripts/mycalibration_function2.R")
attach(msk_wide_data)

# Model w/ all 187 obs (app)
## Discrimination
c_app <- c_index_2$c.index
...
for(i in 1:length(bootIndex)) {
  ...
  bag_surv_model <- intercept + age*bag$DxAge + psa*bag$PreDxBxPSA +
    ...
    pgg2*bag$biop_grp_2 + pgg3*bag$biop_grp_3 + pgg4*bag$biop_grp_4 +
    pgg5*bag$biop_grp_5 + cs_2a*bag$stage_2A + cs_2b*bag$stage_2B +
    cs_2c*bag$stage_2C + cs_3*bag$stage_3p + ece_coef*bag$ece +
    svi_coef*bag$svi + sms_coef*bag$sms + lni_coef*bag$lni
  
  # Calculate the prediction (bag)
  bag_pred_prob_2 <- 1/(1 + (exp(-bag_surv_model)*2)**(1/scaling_param))
  bag_pred_prob_5 <- 1/(1 + (exp(-bag_surv_model)*5)**(1/scaling_param))
  
  # Add predictions to dataset (bag)
  bag_pred_data <- bag %>%
    select(BCR_FreeTime, BCR_Event)
  bag_pred_data$surv2 <- bag_pred_prob_2
  bag_pred_data$surv5 <- bag_pred_prob_5
  bag_pred_data$risk <- bag_surv_model
  
  # Create the spline variables (oob)
  oob <- oob %>%
    mutate(psa_k1 = PreDxBxPSA - PSAPreopKnot1,
           psa_k2 = PreDxBxPSA - PSAPreopKnot2,
           psa_k3 = PreDxBxPSA - PSAPreopKnot3,
  ...
  oob_surv_model <- intercept + age*oob$DxAge + psa*oob$PreDxBxPSA +
    ...
    pgg2*oob$biop_grp_2 + pgg3*oob$biop_grp_3 + pgg4*oob$biop_grp_4 +
    pgg5*oob$biop_grp_5 + cs_2a*oob$stage_2A + cs_2b*oob$stage_2B +
    cs_2c*oob$stage_2C + cs_3*oob$stage_3p + ece_coef*oob$ece +
    svi_coef*oob$svi + sms_coef*oob$sms + lni_coef*oob$lni
  
  # Calculate the prediction (oob)
  oob_pred_prob_2 <- 1/(1 + (exp(-oob_surv_model)*2)**(1/scaling_param))
  oob_pred_prob_5 <- 1/(1 + (exp(-oob_surv_model)*5)**(1/scaling_param))
  
  # Add predictions to dataset (oob)
  oob_pred_data <- oob %>%
    select(BCR_FreeTime, BCR_Event)
  oob_pred_data$surv2 <- oob_pred_prob_2
  oob_pred_data$surv5 <- oob_pred_prob_5
  oob_pred_data$risk <- oob_surv_model
  
  # c-index
  bag_c_index_2 <- survcomp::concordance.index(x = 1 - bag_pred_data$surv2,
                               surv.time = bag_pred_data$BCR_FreeTime,
                               surv.event = bag_pred_data$BCR_Event)
  bag_c_index_5 <- survcomp::concordance.index(x = 1 - bag_pred_data$surv5,
                                             surv.time = bag_pred_data$BCR_FreeTime,
                                             surv.event = bag_pred_data$BCR_Event)
  oob_c_index_2 <- survcomp::concordance.index(x = 1 - oob_pred_data$surv2,
                                             surv.time = oob_pred_data$BCR_FreeTime,
                                             surv.event = oob_pred_data$BCR_Event)
  oob_c_index_5 <- survcomp::concordance.index(x = 1 - oob_pred_data$surv5,
                                   surv.time = oob_pred_data$BCR_FreeTime,
                                   surv.event = oob_pred_data$BCR_Event)
  ...
  # store results
  c_boot[i] <- bag_c_index_2$c.index
  c_oob[i] <- oob_c_index_2$c.index
}
...
dis_dat <- data.frame(oob = c_oob,
                      har = c_boot)
roc_dat <- data.frame(time = msk_pred_data$BCR_FreeTime,
                      status = msk_pred_data$BCR_Event,
                      marker = msk_pred_data$risk)
