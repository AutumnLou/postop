...
# 03_boot_postop_comb_uni_las.R
###########################################################################

# Load the dataset
source("scripts/02_data_comb_postop_matrix.R")
...
# Lasso
## Data preparation
x_full <- as.matrix(comb_data[,c(1:14,17:ncol(comb_data))])
...
app_genes_columns <- sort(as.numeric(probes) + 16)

gene_data <- comb_data[, c(1:16, app_genes_columns)]

x_app <- as.matrix(gene_data[,c(1:14,17:ncol(gene_data))])
...
for(i in 1:length(bootIndex)) {
  ...
  genes_columns <- sort(as.numeric(probes) + 16)
  
  bag_gene_data <- bag[, c(1:16, genes_columns)]
  oob_gene_data <- oob[, c(1:16, genes_columns)]
  orig_gene_data <- comb_data[, c(1:16, genes_columns)]
  ...
  # Lasso data prep
  x <- as.matrix(bag_gene_data[,c(1:14,17:ncol(bag_gene_data))])
  y <- Surv(time = bag_gene_data$BCR_FreeTime,
            event = bag_gene_data$BCR_Event)
  test_x <- as.matrix(oob_gene_data[,c(1:14,17:ncol(oob_gene_data))])
  test_y <- Surv(time = oob_gene_data$BCR_FreeTime,
                 event = oob_gene_data$BCR_Event)
  orig_x <- as.matrix(orig_gene_data[,c(1:14,17:ncol(orig_gene_data))])
...
dis_dat <- data.frame(oob = c_oob,
                      har = c_app - o_boot)
roc_dat <- data.frame(time = comb_data$BCR_FreeTime,
                      status = comb_data$BCR_Event,
                      marker = colMeans(haz, na.rm = TRUE))
write.csv(selection.v, 'data/var_select_comb_uni_las.csv')
