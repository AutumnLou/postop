...
colnames(msc_data)[9] <- "time"
colnames(msc_data)[10] <- "status"
# Model w/ all 187 obs (app)
set.seed(123)
rsf_app <- rsf_rfe(msc_data)
...
set.seed(123)
B <- 100
bootIndex <- createResample(factor(msc_data$status), times = B, list = TRUE)
...
for(i in 1:length(bootIndex)) {
  ...
  orig_val <- predict.rfsrc(rsf_boot, newdata = msc_data)
...
dis_dat <- data.frame(oob = c_oob,
                      har = c_app - o_boot)
roc_dat <- data.frame(time = msc_data$time,
                      status = msc_data$status,
                      marker = colMeans(haz, na.rm = TRUE))
write.csv(selection.v, 'data/var_select_co_rsf.csv')
